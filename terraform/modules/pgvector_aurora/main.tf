# =============================================================================
# pgvector Aurora Serverless Module
# =============================================================================
...
resource "aws_secretsmanager_secret" "db_credentials" {
  name                    = "${var.deployment_name}-pgvector-credentials"
  ...
  tags = merge(
    ...
    {
      Name    = "${var.deployment_name}-pgvector-credentials"
      Purpose = "pgvector Database Credentials"
    }
  )
}
...
resource "aws_kms_key" "rds" {
  description             = "KMS key for pgvector Aurora cluster encryption"
  ...
  tags = merge(
    var.tags,
    {
      Name    = "${var.deployment_name}-pgvector-kms"
...
resource "aws_kms_alias" "rds" {
  name          = "alias/${var.deployment_name}-pgvector"
...
resource "aws_rds_cluster" "pgvector" {
  ...
  final_snapshot_identifier = var.environment == "prod" ? "${var.deployment_name}-pgvector-final-${formatdate("YYYY-MM-DD-hhmmss", timestamp())}" : null
  ...
  tags = merge(
    var.tags,
    {
      Name        = "${var.deployment_name}-pgvector"
...
resource "aws_rds_cluster_instance" "pgvector" {
  ...
  tags = merge(
    var.tags,
    {
      Name  = "${var.deployment_name}-pgvector-${count.index}"
      Index = count.index
    }
  )
}
...
resource "aws_db_subnet_group" "pgvector" {
  name       = "${var.deployment_name}-pgvector-subnet-group"
...
resource "aws_security_group" "pgvector" {
  ...
  tags = merge(
    var.tags,
    {
      Name = "${var.deployment_name}-pgvector-sg"
    }
  )
...
resource "aws_iam_role" "lambda_init" {
  name = "${var.deployment_name}-pgvector-lambda-init"
...
resource "aws_lambda_function" "init_pgvector" {
  ...
  tags = merge(
    var.tags,
    {
      Name    = "${var.deployment_name}-init-pgvector"
      Purpose = "pgvector Extension Initialization"
    }
  )
}
