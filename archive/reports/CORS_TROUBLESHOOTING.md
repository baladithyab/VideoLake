...
This guide helps resolve Cross-Origin Resource Sharing (CORS) issues between the React frontend (http://localhost:5172) and FastAPI backend (http://localhost:8000).
...
app.add_middleware(
    ...
    allow_origins=[
        ...
        "http://localhost:5172",  # Vite dev server (default)
        "http://127.0.0.1:5172",  # Alternative localhost
...
3. **Test CORS headers**:
   ...
   ./test_cors.sh
   ```
   Should show `access-control-allow-origin: http://localhost:5172`
...
```
access-control-allow-origin: http://localhost:5172
...
# INFO: Origin: http://localhost:5172
...
If `http://localhost:5172` doesn't work, try:

1. `http://127.0.0.1:5172`
...
- [ ] Frontend is running on http://localhost:5172
- [ ] CORS middleware is configured in `src/api/main.py`
- [ ] `allow_origins` includes `http://localhost:5172`
